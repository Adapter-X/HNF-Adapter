...
class Convchor(nn.Module):
    def __init__(self, ch_in, f, ch_out, stride, r, padding=1, act_layer=nn.ReLU, norm_layer=partial(nn.BatchNorm2d, eps=1e-6)):
        ...
        self.depthwise_conv = nn.Conv2d(f, f, kernel_size=3, stride=stride, padding=padding, groups=r)
...
class Bridge_ct(nn.Module):
    def __init__(self, outplanes, embed_dim, m=8, l=8, a=1, act_layer=nn.GELU):
        super().__init__()
        self.embed_dim = embed_dim
        self.D = int(math.sqrt(embed_dim))
        ...
        # 升维
        self.adapter_up = nn.Conv2d(m, 197, 1, 1, 0)
        nn.init.xavier_uniform_(self.adapter_up.weight)
    ...
    def forward(self, x_r):
        B, C, H, W = x_r.shape
        H_l = find_nearest(self.D, H)
        x_r = self.adapter_down(x_r)
        x_r = self.bn1(x_r)
        x_r = self.act1(x_r)

        x_r = resize_conv2d(x_r, (H_l, H_l), xavier=True)
        ...
        device = x_r.device
        adapter_fc_down = nn.Linear(H_l * H_l, self.l).to(device)
        adapter_fc_up = nn.Linear(self.l, self.embed_dim).to(device)
        x_r = adapter_fc_down(x_r)
        x_r = self.act2(x_r)
        x_r = self.drop(x_r)
        x_r = adapter_fc_up(x_r)
        x_r = self.drop(x_r)
...
class Bridge_tc(nn.Module):
    def __init__(self, outplanes, embed_dim, m=8, l=8, a=1, act_layer=nn.GELU):
        super().__init__()
        self.H = 7 * (512 // outplanes)

        self.embed_dim = embed_dim
        self.D = int(math.sqrt(embed_dim))

        self.H_l = find_nearest(self.D, self.H)
        self.adapter_fc_down = nn.Linear(self.embed_dim, l)
        self.adapter_fc_up = nn.Linear(l, self.H_l * self.H_l)

        # 降维
        self.adapter_down = nn.Conv2d(197, m, 1, 1, 0)
        nn.init.xavier_uniform_(self.adapter_down.weight)
        self.bn1 = nn.BatchNorm2d(m)
        self.act1 = act_layer()

        # 升维
        self.adapter_up = nn.Conv2d(m, outplanes, 1, 1, 0)
        nn.init.xavier_uniform_(self.adapter_up.weight)
    ...
    def forward(self, x_t):
        B, N, D = x_t.shape
        x_t = self.adapter_fc_down(x_t)
        x_t = self.act2(x_t)
        x_t = self.drop(x_t)
        x_t = self.adapter_fc_up(x_t)
        x_t = self.drop(x_t)

        x_t = x_t.view(B, 197, self.H_l, self.H_l)
        x_t = self.adapter_down(x_t)
        x_t = self.bn1(x_t)
        x_t = self.act1(x_t)
...
class Small_34_Bridge(nn.Module):
    def __init__(self, cnn, vit, m=8, l=8, a=1, embed_dim=768):
        # Transformer
        super().__init__()
        ...
        self.adapter_ct1 = Bridge_ct(64, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct2 = Bridge_ct(128, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct3 = Bridge_ct(128, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct4 = Bridge_ct(128, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct5 = Bridge_ct(128, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct6 = Bridge_ct(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct7 = Bridge_ct(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct8 = Bridge_ct(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct9 = Bridge_ct(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct10 = Bridge_ct(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct11 = Bridge_ct(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_ct12 = Bridge_ct(512, embed_dim=embed_dim, m=m, l=l, a=a)

        self.adapter_tc1 = Bridge_tc(64, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc2 = Bridge_tc(128, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc3 = Bridge_tc(128, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc4 = Bridge_tc(128, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc5 = Bridge_tc(128, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc6 = Bridge_tc(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc7 = Bridge_tc(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc8 = Bridge_tc(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc9 = Bridge_tc(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc10 = Bridge_tc(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc11 = Bridge_tc(256, embed_dim=embed_dim, m=m, l=l, a=a)
        self.adapter_tc12 = Bridge_tc(512, embed_dim=embed_dim, m=m, l=l, a=a)
...
class Resnet34_ViT_S(nn.Module):
    """
    Resnet50和ViT-B/16
    ...
    def __init__(self, f, h, m, l, r, s, a):
        super().__init__()
        self.resnet34 = create_model('resnet34',
                                     checkpoint_path='/media/dl_shouan/ZHITAI/Adapter_test/mmseg_custom/pre_weight/resnet34.pth',
                                     drop_path_rate=0.1)

        self.vit_model = create_model('vit_small_patch16_224',
                                      checkpoint_path='/media/dl_shouan/ZHITAI/Adapter_test/mmseg_custom/pre_weight/ViT-S_16.npz',
                                      drop_path_rate=0.1)
        # torch.save(self.vit_model_1.state_dict(), './vit.pth')
        # vit_dict = torch.load('./vit.pth')
        #
        # self.vit_model = timm.models.vision_transformer.VisionTransformer(img_size=512, patch_size=32, embed_dim=384,
        #                                                                   depth=12, num_heads=6, drop_path_rate=0.1)
        # self.vit_model.load_state_dict(vit_dict)

        embed_dim = self.vit_model.embed_dim
        set_adapter(self.resnet34, "Convchor", embed_dim=embed_dim, f=f, h=h, r=r, s=s)
        set_adapter(self.vit_model, "Transchor", f=f, h=h, embed_dim=embed_dim, r=r, s=s)

        self.model = Small_34_Bridge(cnn=self.resnet34, vit=self.vit_model, m=m, l=l, a=a,
                                     embed_dim=embed_dim)
...
if __name__ == '__main__':
    model = Resnet34_ViT_S(f=32, h=32, m=32, l=32, r=1, s=0.1, a=1)
