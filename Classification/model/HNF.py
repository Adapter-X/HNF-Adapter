...
class Base_50_Bridge(nn.Module):
    def __init__(self, cnn, vit, m=8, l=8, a=1, b=1, embed_dim=768, num_classes=100, N_H=14):
        # Transformer
        super().__init__()
        ...
        self.global_pool, self.fc = create_classifier(2048, num_classes, pool_type='avg')
    ...
    def forward(self, x):
        ...
        # conv classification
        x_p = self.global_pool(x_r)
        conv_cls = self.fc(x_p)
...
class Resnet50_ViT_B(nn.Module):
    ...
    def __init__(self, f, h, m, l, r, s, a, b, num_classes=100, img_size=224):
        super().__init__()
        self.resnet50 = create_model('resnet50', num_classes=11221, drop_path_rate=0.1)
        self.resnet50 = load_model_weights(self.resnet50, '/media/dl_shouan/DATA/fxl/pre_weight/resnet50_21k.pth')
        ...
        self.model = Base_50_Bridge(cnn=self.resnet50, vit=self.vit_model, m=m, l=l, a=a, b=b,
                                     embed_dim=embed_dim, num_classes=num_classes, N_H = N_H)
...
if __name__ == '__main__':
    model = Resnet50_ViT_B(f=8, h=8, m=8, l=8, r=0.1, s=0.1, a=0.1, b=1, num_classes=100, img_size=224)
